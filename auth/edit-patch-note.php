...
// Get patch note ID from URL
$patch_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

if ($patch_id <= 0) {
    header('Location: manage-patch-notes.php');
    exit();
}

// Get existing patch note data
$stmt = $conn->prepare("SELECT * FROM patch_notes WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($version) || empty($title) || empty($release_date) || empty($content)) {
        ...
        // Check if version already exists (excluding current patch note)
        $check_stmt = $conn->prepare("SELECT id FROM patch_notes WHERE version = ? AND id != ?");
        $check_stmt->bind_param("si", $version, $patch_id);
        $check_stmt->execute();
        if ($check_stmt->get_result()->num_rows > 0) {
            $error = 'A patch note with this version already exists.';
        } else {
            // Handle image upload
            $image_url = $patch_note['image_url']; // Keep existing image by default
            ...
            if (empty($error)) {
                // Update patch note
                $stmt = $conn->prepare("UPDATE patch_notes SET version = ?, title = ?, release_date = ?, image_url = ?, content = ? WHERE id = ?");
                $stmt->bind_param("sssssi", $version, $title, $release_date, $image_url, $content, $patch_id);
                
                if ($stmt->execute()) {
                    $message = 'Patch note updated successfully!';
                    // Refresh patch note data
                    $stmt = $conn->prepare("SELECT * FROM patch_notes WHERE id = ?");
                    $stmt->bind_param("i", $patch_id);
                    $stmt->execute();
                    $result = $stmt->get_result();
...
<head>
    ...
    <title>Edit Patch Note | GameTracker.gg</title>
    ...
    <div class="form-container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 style="font-family: 'Orbitron', sans-serif;">
                <span style="color: var(--primary-color);">Edit</span> Patch Note
            </h2>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="version" name="version" value="<?= htmlspecialchars($patch_note['version']) ?>" required>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control" id="release_date" name="release_date" value="<?= htmlspecialchars($patch_note['release_date']) ?>" required>
                    </div>
                </div>

                <div class="mb-3">
                    <label for="title" class="form-label">Title *</label>
                    <input type="text" class="form-control" id="title" name="title" value="<?= htmlspecialchars($patch_note['title']) ?>" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <?php if (!empty($patch_note['image_url'])): ?>
                        <div class="mt-2">
                            <strong>Current Image:</strong><br>
                            <img src="../<?= htmlspecialchars($patch_note['image_url']) ?>" alt="Current patch note image" class="current-image">
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="content" name="content" required rows="15"><?= htmlspecialchars($patch_note['content']) ?></textarea>
                    <div class="version-helper">
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-check-circle"></i> Update Patch Note
                    </button>
